...
function calculateStats() {
    let distCount = 0;
    let citySet = new Set();
    const brandCount = Object.keys(distributorsData).length; // 20 marcas oficiales
...
const brandIcons = {
    "AMSTED RAIL": "fa-solid fa-train",
    "CREANZA": "fa-solid fa-layer-group",
    "DÖNMEZ": "fa-solid fa-car-side",
    "F.S.S.": "fa-solid fa-screwdriver-wrench",
    "GATES": "fa-solid fa-link",
    "GEDORE": "fa-solid fa-wrench",
    "HOSE SOLUTIONS": "fa-solid fa-pipe-valve",
    "KETTENWULF": "fa-solid fa-link",
    "KIT MASTERS": "fa-solid fa-fan",
    "MINER": "fa-solid fa-cubes",
    "MWE": "fa-solid fa-bolt",
    "PRESTONE": "fa-solid fa-droplet",
    "RAMSEY PRODUCTS": "fa-solid fa-link",
    "RELIABLE": "fa-solid fa-shield-halved",
    ...
    "WALKER PRODUCTS": "fa-solid fa-gear",
...
// Render GATES agrupado por nombre de distribuidor
function renderGatesByName(cities) {
    ...
    citiesToShow.forEach(city => {
        ...
        if (!cityData) return;
        cityData.distributors.forEach(d => {
            if (searchTerm) {
                const s = searchTerm.toLowerCase();
                if (!d.name.toLowerCase().includes(s) && !d.phone.toLowerCase().includes(s) && !city.toLowerCase().includes(s)) return;
            }
            if (!byName[d.name]) byName[d.name] = [];
            byName[d.name].push({ city, region: cityData.region, phone: d.phone });
        });
    });

    const names = Object.keys(byName).sort((a, b) => a.localeCompare(b));
    if (names.length === 0) return { html: '', count: 0 };

    const cardsHtml = names.map(name => {
        const locations = byName[name].map(l => `
            <div class="dc-byname-location">
                <span class="dc-byname-city"><i class="fa-solid fa-location-dot"></i> ${l.city}</span>
                <span class="dc-byname-region">${l.region}</span>
                <a href="tel:${l.phone.replace(/[^0-9+]/g, '')}" class="dc-byname-phone"><i class="fa-solid fa-phone"></i> ${l.phone}</a>
            </div>
        `).join('');

        return `
            <div class="dc-byname-card">
                <div class="dc-dist-avatar">${getInitials(name)}</div>
                <div class="dc-byname-body">
                    <div class="dc-byname-name">${name}</div>
                    <div class="dc-byname-locations">${locations}</div>
                </div>
            </div>
        `;
    }).join('');

    return { html: `<div class="dc-byname-grid">${cardsHtml}</div>`, count: names.length };
}
...
function renderDistributors() {
    ...
    brandsToShow.forEach(brand => {
        ...
        // Marcas sin distribuidores registrados aún
        if (Object.keys(cities).length === 0) {
            if (activeBrand === brand) {
                html += `
                    <div class="dc-brand-section">
                        <div class="dc-brand-header">
                            <div class="dc-brand-logo"><i class="${brandIcons[brand] || 'fa-solid fa-tags'}"></i></div>
                            <span class="dc-brand-name">${brand}</span>
                        </div>
                        <div class="dc-no-results" style="padding:40px 20px;">
                            <i class="fa-solid fa-clock"></i>
                            <h3>Próximamente</h3>
                            <p>Estamos actualizando la red de distribuidores para esta marca.<br>Contáctanos para más información.</p>
                        </div>
                    </div>
                `;
            }
        ...
        // GATES: vista agrupada por nombre de distribuidor
        if (brand === "GATES") {
            const { html: gatesHtml, count } = renderGatesByName(cities);
            if (count > 0) {
                ...
                html += `
                    <div class="dc-brand-section">
                        <div class="dc-brand-header">
                            ...
                            <span class="dc-brand-name">${brand}</span>
                            <span class="dc-brand-count">${count} distribuidor${count !== 1 ? 'es' : ''}</span>
                        </div>
                        ${gatesHtml}
                    </div>
                `;
